...
if uploaded_file is not None:
    ...
    score_acc = round(accuracy_score(bank_test['result'], input_df[0])*100, 4)
    score_rec = round(recall_score(bank_test['result'], input_df[0])*100, 4)
    score_pre = round(precision_score(bank_test['result'], input_df[0])*100, 4)
    score_f1 = round(f1_score(bank_test['result'], input_df[0])*100, 4)
    score_auc = round(roc_auc_score(bank_test['result'], input_df[0])*100, 4)
